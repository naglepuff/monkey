...
import (
	...
	"github.com/naglepuff/monkey/ast"
	"github.com/naglepuff/monkey/lexer"
)
...
`
	...
	l := lexer.New(input)
	p := New(l) // we are in the parser package
...
func TestIntegerLiteralExpression(t *testing.T) {
	...
	if integerLiteral.Value != 5 {
		t.Errorf("ident.Value not %d. got=%d", 5, integerLiteral.Value)
	}
	if integerLiteral.TokenLiteral() != "5" {
		t.Errorf("ident.TokenLiteral not %s. got=%s", "5", integerLiteral.TokenLiteral())
	}
}
